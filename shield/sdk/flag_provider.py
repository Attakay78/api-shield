"""ShieldSDKFlagProvider — OpenFeature provider for SDK clients.

Syncs feature flags and segments from a remote Shield Server:

1. On ``initialize()`` it fetches all flags/segments via REST
   (``GET /api/flags`` and ``GET /api/segments``).
2. It then subscribes to the Shield Server's SSE stream
   (``GET /api/sdk/events``) so any subsequent flag/segment mutations
   made on the server side are reflected locally with no polling.

Use ``ShieldSDK.use_openfeature()`` to activate this provider.
"""
...
from shield.core.feature_flags.models import FeatureFlag, Segment  # noqa: E402
from shield.core.feature_flags.provider import ShieldOpenFeatureProvider  # noqa: E402
...
__all__ = ["ShieldSDKFlagProvider"]


class ShieldSDKFlagProvider(ShieldOpenFeatureProvider):
    """OpenFeature provider that hot-reloads flags from a Shield Server.

    Parameters
    ----------
    backend:
        The :class:`~shield.core.backends.server.ShieldServerBackend`
        instance used by this SDK — the same one passed to
        :class:`~shield.sdk.ShieldSDK`.
    """

    def __init__(self, backend: ShieldServerBackend) -> None:
    ...
    def initialize(self, evaluation_context: Any = None) -> None:
        ...
        loop.create_task(self._async_initialize(), name="shield-sdk-flag-init")

    async def _async_initialize(self) -> None:
        await self._fetch_from_server()
        self._watch_task = asyncio.create_task(self._watch_sse(), name="shield-sdk-flag-watch")
    ...
    async def _fetch_from_server(self) -> None:
        """Pull current flags and segments from the Shield Server REST API."""
        client = self._server_backend._client
        if client is None:
            logger.warning(
                "ShieldSDKFlagProvider: HTTP client not ready — skipping initial flag fetch"
            )
            return
        ...
        try:
            ...
            logger.warning("ShieldSDKFlagProvider: could not fetch flags from server")
        ...
        try:
            ...
            logger.warning("ShieldSDKFlagProvider: could not fetch segments from server")

        logger.info(
            "ShieldSDKFlagProvider: loaded %d flag(s), %d segment(s) from server",
            len(self._flags),
            len(self._segments),
        )
                ...
                if etype == "flag_updated":
                    ...
                    if raw is not None:
                        try:
                            ...
                            self._flags[flag.key] = flag
                            logger.debug("ShieldSDKFlagProvider: flag hot-reloaded — %s", flag.key)
                        except Exception:  # noqa: BLE001
                            pass
                    ...
                    logger.debug("ShieldSDKFlagProvider: flag removed — %s", key)
                    ...
                    if raw is not None:
                        try:
                            ...
                            self._segments[seg.key] = seg
                            logger.debug(
                                "ShieldSDKFlagProvider: segment hot-reloaded — %s", seg.key
                            )
                        except Exception:  # noqa: BLE001
                            pass
                    ...
                    logger.debug("ShieldSDKFlagProvider: segment removed — %s", key)

        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("ShieldSDKFlagProvider: SSE watch error")
