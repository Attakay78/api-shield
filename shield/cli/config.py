"""CLI configuration — manages ``~/.shield/config.json``.

Provides cross-platform helpers for reading and writing the Shield CLI
...
.. code-block:: json
    ...
    {
        "server_url": "http://localhost:8000/shield",
        "auth": {
            "token": "<signed-token>",
            "username": "admin",
...
3. ``server_url`` key in ``~/.shield/config.json``
4. Built-in default: ``http://localhost:8000/shield``

The ``.shield`` file uses the same key name as the environment variable::
...
_DEFAULT_SERVER_URL = "http://localhost:8000/shield"


def get_config_dir() -> Path:
    """Return the platform-appropriate Shield config directory.

    The directory is created if it does not already exist.
    """
    system = platform.system()
    if system == "Windows":
        base = Path.home() / "AppData" / "Local" / "shield"
    else:
        base = Path.home() / ".shield"
...
def find_shield_file(start: Path | None = None) -> Path | None:
    """Walk up the directory tree from *start* looking for a ``.shield`` file.

    Returns the first ``.shield`` file found, or ``None`` if none exists.
    *start* defaults to the current working directory.
    """
    current = (start or Path.cwd()).resolve()
    while True:
        candidate = current / ".shield"
...
def _parse_shield_file(path: Path) -> dict[str, str]:
    """Parse a ``.shield`` file into a ``{KEY: value}`` dict.
...
def get_server_url() -> str:
    ...
    1. ``SHIELD_SERVER_URL`` environment variable
    2. ``SHIELD_SERVER_URL`` key in a ``.shield`` file (walked up from cwd)
    3. ``server_url`` key in ``~/.shield/config.json``
    4. Built-in default: ``http://localhost:8000/shield``
    """
    # 1. Environment variable
    env_url = os.environ.get("SHIELD_SERVER_URL", "").strip()
    if env_url:
        return env_url.rstrip("/")

    # 2. .shield file in the project tree
    shield_file = find_shield_file()
    if shield_file:
        pairs = _parse_shield_file(shield_file)
        file_url = pairs.get("SHIELD_SERVER_URL", "").strip()
        if file_url:
...
def get_server_url_source() -> str:
    ...
    if os.environ.get("SHIELD_SERVER_URL", "").strip():
        return "env SHIELD_SERVER_URL"
    shield_file = find_shield_file()
    if shield_file:
        pairs = _parse_shield_file(shield_file)
        if pairs.get("SHIELD_SERVER_URL", "").strip():
            return f".shield ({shield_file})"
...
def set_server_url(url: str) -> None:
    """Save *url* as the admin server URL in ``~/.shield/config.json``."""
    cfg = load_config()
