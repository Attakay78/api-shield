"""SwitchlyMiddleware — ASGI middleware that enforces route lifecycle policies.

Every inbound request passes through ``dispatch()``.  The middleware:

1. Scans all app routes for ``__switchly_meta__`` at ASGI lifespan startup so
...
4. Translates ``SwitchlyException`` subclasses into structured JSON responses.
5. Injects RFC headers for deprecated routes and ``Retry-After`` for
   maintenance responses.

Route scanning — how and when it happens
-----------------------------------------
``SwitchlyMiddleware`` hooks into the ASGI lifespan to scan *all* app routes for
``__switchly_meta__`` immediately after the server's startup events complete
(which means ``SwitchlyRouter.on_startup`` hooks have already run).
...
# Prefixes that are always exempt from switchly checks.
_SKIP_PREFIXES = ("/switchly/", "/docs", "/redoc", "/openapi.json")


class SwitchlyMiddleware(BaseHTTPMiddleware):
    ...
    responses:
        Optional mapping of switchly status → response factory used as the
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        event handlers (including ``SwitchlyRouter.on_startup``) have already
            ...
            if app is not None:
                ...
                async def _send(message: Any) -> None:
                    if message.get("type") == "lifespan.startup.complete":
                        ...
                        # after all startup events (SwitchlyRouter hooks, etc.)
                        # have already run.
                        await self._do_scan(app)
                        self.engine.scheduler.start_polling()
                        # Make the engine discoverable by decorator deps via
                        # request.app.state.switchly_engine — this is what lets
                        # maintenance()/disabled()/env_only() find the engine
                        # without needing engine= on every call.
                        from switchly.fastapi.dependencies import configure_switchly

                        configure_switchly(app, self.engine)
    ...
    def _build_route_cache(self, app: Any) -> None:
        ...
        * ``_param_routes`` — parameterised routes (e.g. ``/items/{id}``).
        ...
        for route in getattr(app, "routes", []):
            ...
            meta = getattr(endpoint, "__switchly_meta__", {}) if endpoint else {}
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        """Intercept the request and run switchly checks."""
        ...
        # Always skip internal/docs prefixes for switchly enforcement.
        ...
        if is_force_active:
            # @force_active routes bypass all switchly checks by default.
    ...
    def _resolve_route(self, request: Request) -> tuple[bool, str | None, Any, Any]:
        ...
        - ``is_force_active`` — ``True`` when the matched endpoint carries
          ``@force_active`` and should bypass all switchly checks.
    ...
    async def _call_response_factory(factory: Any, request: Request, exc: Exception) -> Response:
        ...
        The factory receives the live ``Request`` and the ``SwitchlyException``
