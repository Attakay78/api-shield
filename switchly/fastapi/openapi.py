"""OpenAPI schema filter and docs UI customisation for switchly.

``apply_switchly_to_openapi`` monkey-patches ``app.openapi()`` so that the
...
- ``MAINTENANCE`` — ``x-switchly-status`` extension added; markdown warning
                    prepended to the operation description; summary prefixed
                    with ``🔧 ``.
- **Global maintenance ON** — ``x-switchly-global-maintenance`` extension added
...
``setup_switchly_docs`` replaces both ``/docs`` (Swagger UI) and ``/redoc``
...
from switchly.core.models import GlobalMaintenanceConfig, RouteState, RouteStatus
...
_MAINTENANCE_DESCRIPTION_BANNER = (
    "> 🔧 **Switchly: MAINTENANCE** — {reason}\n>\n"
    "> This endpoint is temporarily unavailable.  "
    "Check back later or contact your API team.\n\n"
)


def apply_switchly_to_openapi(app: FastAPI, engine: SwitchlyEngine) -> None:
    """Patch ``app.openapi()`` to filter paths based on switchly route state.
        ...
        if global_cfg is not None and global_cfg.enabled:
            ...
            for path in list(filtered.keys()):
                ...
                for key, value in path_item.items():
                    ...
                    if value.get("x-switchly-status") == "maintenance":
                        patched_for_global[key] = value
                        continue
            ...
            # The sticky HTML banner (rendered by setup_switchly_docs) is the
            ...
            schema["info"]["x-switchly-global-maintenance"] = {
                "enabled": True,
            ...
            schema["info"] = dict(schema.get("info", {}))
            schema["info"]["x-switchly-global-maintenance"] = {"enabled": False}
...
def _annotate_maintenance(operation: dict[str, Any], state: RouteState) -> dict[str, Any]:
    ...
    * ``x-switchly-status`` / ``x-switchly-reason`` — machine-readable extensions
      picked up by ``setup_switchly_docs`` to apply visual styling in Swagger UI.
    ...
    * ``summary`` — ``🔧 `` prefix added so the maintenance state is visible
    ...
    patched["x-switchly-status"] = "maintenance"
    patched["x-switchly-reason"] = state.reason or ""
...
#
# Responsibilities:
#  1. Read ``info['x-switchly-global-maintenance']`` from the live OpenAPI spec.
...
#  4. Add per-operation orange badges for routes with x-switchly-status=maintenance.
...
<style>
  ...
  #switchly-global-banner {
    ...
    animation: switchly-pulse 2.5s ease-in-out infinite;
  }
  @keyframes switchly-pulse {
    0%, 100% { box-shadow: 0 4px 16px rgba(198,40,40,0.5); }
    50%       { box-shadow: 0 4px 32px rgba(198,40,40,0.9); }
  }
  #switchly-global-banner .switchly-banner-title {
    font-size: 22px;
    font-weight: 900;
    letter-spacing: 2px;
  ...
  #switchly-global-banner .switchly-banner-reason {
    font-size: 15px;
    opacity: 0.92;
    margin-bottom: 4px;
  }
  #switchly-global-banner .switchly-banner-exempt {
    font-size: 12px;
    opacity: 0.75;
    font-style: italic;
  }
  ...
  #switchly-ok-chip .dot {
    width: 8px;
    height: 8px;
    border-radius: 50%;
    background: #69f0ae;
    animation: switchly-blink 2s ease-in-out infinite;
  }
  @keyframes switchly-blink {
    0%, 100% { opacity: 1; }
    50%       { opacity: 0.3; }
  }

  /* --- Per-operation maintenance styling --- */
  .switchly-maintenance-block {
    border-left: 4px solid #ff9800 !important;
    background: rgba(255, 152, 0, 0.07) !important;
  }
  .switchly-maintenance-badge {
...
(function () {
  ...
  function applyGlobalBanner(spec) {
    var gm = (spec.info || {})['x-switchly-global-maintenance'] || {};
    var bannerEl = document.getElementById('switchly-global-banner');
    var chipEl   = document.getElementById('switchly-ok-chip');
    ...
    if (gm.enabled) {
      ...
      if (!bannerEl) {
        ...
        bannerEl.id = 'switchly-global-banner';
      ...
      var exemptHtml = exempts.length
        ? '<div class="switchly-banner-exempt">Exempt: ' +
          exempts.join(' &bull; ') + '</div>'
        : '';

      bannerEl.innerHTML =
        '<div class="switchly-banner-title">🔴 Site-Wide Maintenance In Progress</div>' +
        '<div class="switchly-banner-reason">' + escHtml(reason) + '</div>' +
        exemptHtml;
  ...
  function buildMaintenanceMap(spec) {
    ...
    Object.keys(paths).forEach(function (path) {
      ...
      ['get','post','put','patch','delete','head','options'].forEach(function (m) {
        var op = methods[m];
        if (op && op['x-switchly-status'] === 'maintenance') {
          map[m.toUpperCase() + ':' + path] = op['x-switchly-reason'] || '';
        }
      });
    });
    return map;
  ...
  function applyBadges(maintenanceMap) {
    document.querySelectorAll('.opblock').forEach(function (el) {
      ...
      if (el.classList.contains('switchly-maintenance-block')) return;

      el.classList.add('switchly-maintenance-block');

      var summary = el.querySelector('.opblock-summary');
      if (summary && !summary.querySelector('.switchly-maintenance-badge')) {
        var badge = document.createElement('span');
        badge.className   = 'switchly-maintenance-badge';
...
def setup_switchly_docs(app: FastAPI, engine: SwitchlyEngine) -> None:
    """Replace the default ``/docs`` and ``/redoc`` with switchly-aware versions.
    ...
    * **Per-route maintenance** — orange left-border + ``🔧 MAINTENANCE``
    ...
    (CLI ``switchly global enable/disable``) without a page reload.

    Call *after* ``apply_switchly_to_openapi``::
    ...
    def _inject(base_html: str) -> str:
        """Embed openapi URL + switchly script into an HTML page."""
        base_html = base_html.replace(
            "<body>",
            f'<body data-openapi-url="{openapi_url}">',
            1,
        )
        return base_html.replace("</body>", f"{_SWITCHLY_DOCS_SCRIPT}\n</body>", 1)

    @app.get(docs_url, include_in_schema=False)
    async def switchly_swagger_ui() -> HTMLResponse:
    ...
    async def switchly_redoc() -> HTMLResponse:
...
def _fetch_states(engine: SwitchlyEngine) -> list[RouteState] | None:
...
def _fetch_global_config(engine: SwitchlyEngine) -> GlobalMaintenanceConfig | None:
