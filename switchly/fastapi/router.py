"""SwitchlyRouter — drop-in replacement for FastAPI's APIRouter.

At application startup it scans every route for ``__switchly_meta__``
...
directly on the ``FastAPI`` app to benefit from switchly decorators too.
"""
...
async def _register_rate_limit_from_meta(
    path: str, method: str, meta: dict[str, Any], engine: SwitchlyEngine
) -> None:
    """Register a rate limit policy from ``__switchly_meta__`` if present.
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            "switchly: failed to register rate limit for %s %s: %s", method, path, exc
        )


async def scan_routes(app: Any, engine: SwitchlyEngine) -> None:
    ...
    **All routes are registered**, not just decorated ones:
    ...
    - Routes with ``__switchly_meta__`` are registered with their decorator state.
    - Routes without ``__switchly_meta__`` are registered as ``ACTIVE``.
    ...
    ``SwitchlyRouter`` startup hook or a previous ``scan_routes()`` call) are
    ...
    app:
        ...
        Inside ``SwitchlyMiddleware`` this is ``request.app``.
    engine:
    ...
    for route in getattr(app, "routes", []):
        ...
        # user-defined routes and should never appear in switchly status.
        ...
        meta: dict[str, Any] = getattr(endpoint, "__switchly_meta__", {})
...
class SwitchlyRouter(APIRouter):
    """Drop-in replacement for ``fastapi.APIRouter``.

    Identical to ``APIRouter`` in every way except that it registers
    switchly metadata with the engine at startup.

    Parameters
    ----------
    engine:
        The ``SwitchlyEngine`` instance shared with the middleware.
    **kwargs:
        All other keyword arguments are forwarded to ``APIRouter``.
    """

    def __init__(self, engine: SwitchlyEngine, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self._switchly_engine = engine
        # Collect (path, meta) pairs discovered via add_api_route.
        self._switchly_routes: list[tuple[str, dict[str, Any]]] = []
        # Collect (path, method, meta) triples for rate limit registration.
        self._switchly_rl_routes: list[tuple[str, str, dict[str, Any]]] = []
        # Register the startup hook so FastAPI picks it up automatically.
        self.on_startup.append(self.register_switchly_routes)
    ...
    @classmethod
    def from_engine(cls, engine: SwitchlyEngine, **kwargs: Any) -> SwitchlyRouter:
        """Create a ``SwitchlyRouter`` bound to *engine*."""
        return cls(engine=engine, **kwargs)

    # ------------------------------------------------------------------
    # Override route registration to detect __switchly_meta__
    # ------------------------------------------------------------------
    ...
    def add_api_route(
        ...
        **kwargs: Any,
        ...
        """Register a route and record any switchly metadata on the endpoint.
        ...
        ``SwitchlyMiddleware`` resolves via template-path lookup.  For example,
        ...
        if methods:
            ...
            for method in sorted(methods):
                self._switchly_routes.append((f"{method}:{full_path}", meta))
                # Collect rate limit registrations for startup.
                if meta.get("rate_limit"):
                    self._switchly_rl_routes.append((full_path, method, meta))
        else:
            # No methods known — fall back to path-level (all-methods) key.
            self._switchly_routes.append((full_path, meta))
            if meta.get("rate_limit"):
                self._switchly_rl_routes.append((full_path, "ALL", meta))

    # ------------------------------------------------------------------
    # Startup: register all discovered routes with the engine
    # ------------------------------------------------------------------

    async def register_switchly_routes(self) -> None:
        """Register all switchly-decorated routes with the engine.

        Call this during application startup (e.g. via a ``lifespan``
        handler or ``on_startup`` event).  ``SwitchlyRouter`` calls this
        ...
        for path, method, meta in self._switchly_rl_routes:
            await _register_rate_limit_from_meta(path, method, meta, self._switchly_engine)
        await self._switchly_engine.register_batch(list(self._switchly_routes))
        await self._switchly_engine.start()
    ...
    def _get_startup_handler(self) -> Callable[[], Any]:
        """Return an async startup handler that registers switchly routes."""

        async def _startup() -> None:
            await self.register_switchly_routes()
