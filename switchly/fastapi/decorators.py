"""FastAPI route decorators for switchly.

Every decorator returns a :class:`_SwitchlyCallable` — a dual-purpose object:

* **As a decorator**: stamps ``__switchly_meta__`` on the route function.
  ``SwitchlyRouter`` reads this at startup and registers the initial state in
...
* **As a FastAPI dependency** (via ``Depends()``): enforces the route state
  ...
  1. **Engine from app state** (recommended — zero config per route):
     Call ``configure_switchly(app, engine)`` once (or use ``SwitchlyMiddleware``
     which does this automatically).  All decorator deps then find the engine
     via ``request.app.state.switchly_engine`` without any ``engine=`` argument.

  2. **Explicit engine** (``engine=engine``): overrides the app-state lookup.
     Useful when you have multiple engines or don't use ``configure_switchly``.
...
All three styles use the same import — no ``_dep`` suffix, no separate class::
    ...
    from switchly.fastapi import maintenance, disabled, env_only, configure_switchly

    # Once, in app setup:
    configure_switchly(app, engine)  # or: app.add_middleware(SwitchlyMiddleware, engine=engine)
...
registered in the backend (either via ``SwitchlyRouter`` at startup, or by
...
# SwitchlyException that triggered the block, and returns a Starlette Response.
...
_SWITCHLY_ENGINE_ATTR = "switchly_engine"
...
def _resolve_engine(explicit: Any, request: Request) -> Any:
    ...
    2. ``request.app.state.switchly_engine`` — set by ``configure_switchly`` or
       ``SwitchlyMiddleware`` at ASGI lifespan startup.
    3. ``None`` — no engine available; caller falls back to inline logic.
    """
    if explicit is not None:
        return explicit
    return getattr(request.app.state, _SWITCHLY_ENGINE_ATTR, None)
...
def _engine_dep_raise(engine: Any, request: Request) -> None:
    ...
    within such a thread.  SwitchlyExceptions are caught here and re-raised
    as ``HTTPException``.
    """
...
def _stamp(func: F, meta: dict[str, Any]) -> F:
    """Attach *meta* as ``__switchly_meta__`` on *func*, merging if already set."""
    existing: dict[str, Any] = getattr(func, "__switchly_meta__", {})
    existing.update(meta)
    func.__switchly_meta__ = existing  # type: ignore[attr-defined]
    return func
...
class _SwitchlyCallable:
    """Dual-purpose callable: decorator or FastAPI dependency.

    When called with a **function** (decorator use), stamps ``__switchly_meta__``
...
def maintenance(
    ...
    engine:
        ...
        ``app.state.switchly_engine`` (set by ``configure_switchly`` or
        ``SwitchlyMiddleware``).  Pass explicitly only when you need to target a
    ...
    Zero-config dep (engine resolved from app state automatically)::
        ...
        configure_switchly(app, engine)  # once
    ...
    return _SwitchlyCallable(meta=meta, dep_raise=dep_raise)
...
def env_only(
    *envs: str, engine: Any = None, response: ResponseFactory | None = None
    ...
    engine:
        Optional explicit engine override.  When omitted, the dep looks up
        ``app.state.switchly_engine``.  If no engine is found anywhere, the dep
        fails open (use ``SwitchlyMiddleware`` for middleware-level enforcement).
    response:
        Optional custom response factory — a sync or async callable with
        signature ``(request: Request, exc: Exception) -> Response``.
...
def disabled(
    ...
    engine:
        Optional explicit engine override.  When omitted, the dep looks up
        ``app.state.switchly_engine``.  When an engine is available the route
        can be re-enabled at runtime via ``switchly enable <path>`` or the
        dashboard.
    response:
        Optional custom response factory — a sync or async callable with
        signature ``(request: Request, exc: Exception) -> Response``.
    ...
    Examples
    --------
    Zero-config dep (re-enable with ``switchly enable /old-endpoint``)::
...
def deprecated(
    ...
    - ``SwitchlyRouter`` registers the route with status ``DEPRECATED``.
...
def rate_limit(
    ...
    *,
    ...
    Works as both a decorator (stamps ``__switchly_meta__``) and a FastAPI
    dependency (enforces the limit at request time when the engine is
    available via ``configure_switchly``).
...
def force_active(func: F) -> F:
    """Force a route to bypass all switchly checks.
    ...
    ``Depends()`` dependency.  Switchly checks are enforced by the middleware,
