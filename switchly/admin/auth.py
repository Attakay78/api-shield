"""Auth backends and token management for SwitchlyAdmin.
...
* ``MyAuthBackend()``             — custom :class:`SwitchlyAuthBackend` subclass
...
class _SingleUserAuth(SwitchlyAuthBackend):
...
class _MultiUserAuth(SwitchlyAuthBackend):
...
def make_auth_backend(auth: AuthConfig) -> SwitchlyAuthBackend | None:
    """Convert *auth* config into a :class:`SwitchlyAuthBackend` instance.
    ...
    auth:
        ...
        * :class:`SwitchlyAuthBackend` instance — custom implementation
    """
    if auth is None:
        return None
    if isinstance(auth, tuple):
...
class TokenManager:
    ...
    COOKIE_NAME = "switchly_session"
    TOKEN_HEADER = "X-Switchly-Token"
    ...
    def extract_token(self, token_header_value: str) -> str | None:
        """Pull raw token from the ``X-Switchly-Token`` header value."""
