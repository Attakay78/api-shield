"""SwitchlyAdmin — unified admin ASGI app.

Replaces :func:`~switchly.dashboard.app.SwitchlyDashboard` as the recommended
entry-point.  A single mount provides:

* **Dashboard UI** — the HTMX / Jinja2 admin panel (same as before)
* **REST API** — JSON endpoints the ``switchly`` CLI uses as its back-end
* **Auth** — token-based session management shared by both surfaces

Usage::

    from switchly.admin import SwitchlyAdmin
...
* :class:`~switchly.admin.auth.SwitchlyAuthBackend` subclass instance — custom

Dashboard sessions
------------------
After logging in via ``/login`` the browser receives an ``HttpOnly`` session
cookie (``switchly_session``).  The actor stored in audit entries is the
authenticated username; platform is ``"dashboard"``.

CLI tokens
----------
``POST /api/auth/login`` returns a bearer token the CLI stores in
``~/.switchly/config.json``.  All subsequent CLI requests send
``X-Switchly-Token: <token>``; the actor is the authenticated username
and platform is ``"cli"``.

No-auth mode
------------
When ``auth=None`` every request is accepted.  The actor defaults to
``"anonymous"`` unless the client sends an ``X-Switchly-Actor`` header, in
...
from switchly.admin import api as _api
from switchly.admin.auth import AuthConfig, TokenManager, auth_fingerprint, make_auth_backend
from switchly.core.engine import SwitchlyEngine
from switchly.dashboard import routes as _dash
...
class _AuthMiddleware(BaseHTTPMiddleware):
    """Inject ``switchly_actor`` and ``switchly_platform`` into ``request.state``.

    * If auth is **not** configured every request passes through; actor is
      taken from the ``X-Switchly-Actor`` header or falls back to
      ``"anonymous"``.
    * If auth **is** configured:

      1. Try ``X-Switchly-Token: <token>`` (CLI / programmatic access).
      2. Try ``switchly_session`` cookie (dashboard browser session).
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if not self._has_auth:
            # No auth — allow everything; honour optional X-Switchly-Actor hint.
            request.state.switchly_actor = (
                request.headers.get("X-Switchly-Actor", "") or "anonymous"
            )
            request.state.switchly_platform = request.headers.get("X-Switchly-Platform", "cli")
        ...
        if any(path == p or path.endswith(p) for p in _PUBLIC_PATHS) or any(
            ...
            request.state.switchly_actor = "anonymous"
            request.state.switchly_platform = "anonymous"
            return await call_next(request)

        # Try X-Switchly-Token header first (CLI / programmatic access).
        token = self._tm.extract_token(request.headers.get("X-Switchly-Token", ""))
        ...
        if token:
            ...
            if result:
                request.state.switchly_actor = result[0]
                request.state.switchly_platform = result[1]
                return await call_next(request)

        # No valid credentials — decide how to reject.
        is_api = "/api/" in path
        is_cli = request.headers.get("X-Switchly-Platform", "") == "cli"
...
def _flag_dashboard_routes() -> list[Route]:
    """Return the flag + segment dashboard UI routes for mounting in SwitchlyAdmin."""
    return [
...
def _flag_routes() -> list[Route]:
    """Return the flag + segment API routes for mounting in SwitchlyAdmin."""
    return [
        # ── Flags CRUD ───────────────────────────────────────────────
...
def SwitchlyAdmin(
    ...
    """Create the unified Switchly admin ASGI app.
    ...
    ----------
    engine:
        The :class:`~switchly.core.engine.SwitchlyEngine` to administer.
    auth:
        Credentials config.  ``None`` = open access.  Accepts a
        ``(username, password)`` tuple, a list of such tuples, or a custom
        :class:`~switchly.admin.auth.SwitchlyAuthBackend` instance.
    token_expiry:
    ...
    enable_flags:
        ...
        ``switchly[flags]`` to be installed.
    ...
    def _clean_entry_path(entry: object) -> str:
        ...
        if raw.startswith("__switchly:svc_global:") and raw.endswith("__"):
            name = raw[len("__switchly:svc_global:") : -2]
            return f"[{name} Maintenance]"
        if raw.startswith("__switchly:svc_rl:") and raw.endswith("__"):
            name = raw[len("__switchly:svc_rl:") : -2]
    ...
    try:
        version = importlib.metadata.version("switchly")
    ...
    starlette_app = Starlette(
        routes=[
            ...
            *(_flag_dashboard_modal_routes() if enable_flags else []),
            ...
            # ── SDK endpoints (SwitchlyServerBackend / SwitchlySDK) ──────────
