"""Switchly dashboard — mountable Starlette admin UI factory."""
...
def SwitchlyDashboard(
    engine: SwitchlyEngine,
    prefix: str = "/switchly",
    auth: tuple[str, str] | None = None,
) -> ASGIApp:
    """Create a mountable Starlette admin UI for the Switchly engine.

    Mount it on any FastAPI / Starlette application::

        app.mount("/switchly", SwitchlyDashboard(engine=engine))
    ...
    ----------
    engine:
        The :class:`~switchly.core.engine.SwitchlyEngine` whose state this
    ...
    prefix:
        ...
        ``app.mount()``.  Defaults to ``"/switchly"``.
    auth:
    ...
    if auth is not None:
        from switchly.dashboard.auth import BasicAuthMiddleware
