"""SwitchlySDKFlagProvider — OpenFeature provider for SDK clients.

Syncs feature flags and segments from a remote Switchly Server:

1. On ``initialize()`` it fetches all flags/segments via REST
   (``GET /api/flags`` and ``GET /api/segments``).
2. It then subscribes to the Switchly Server's SSE stream
   (``GET /api/sdk/events``) so any subsequent flag/segment mutations
   made on the server side are reflected locally with no polling.

Use ``SwitchlySDK.use_openfeature()`` to activate this provider.
"""
...
from switchly.core.feature_flags.models import FeatureFlag, Segment  # noqa: E402
from switchly.core.feature_flags.provider import SwitchlyOpenFeatureProvider  # noqa: E402
...
__all__ = ["SwitchlySDKFlagProvider"]


class SwitchlySDKFlagProvider(SwitchlyOpenFeatureProvider):
    """OpenFeature provider that hot-reloads flags from a Switchly Server.

    Parameters
    ----------
    backend:
        The :class:`~switchly.core.backends.server.SwitchlyServerBackend`
        instance used by this SDK — the same one passed to
        :class:`~switchly.sdk.SwitchlySDK`.
    """

    def __init__(self, backend: SwitchlyServerBackend) -> None:
    ...
    def initialize(self, evaluation_context: Any = None) -> None:
        ...
        loop.create_task(self._async_initialize(), name="switchly-sdk-flag-init")

    async def _async_initialize(self) -> None:
        await self._fetch_from_server()
        self._watch_task = asyncio.create_task(self._watch_sse(), name="switchly-sdk-flag-watch")
    ...
    async def _fetch_from_server(self) -> None:
        """Pull current flags and segments from the Switchly Server REST API."""
        client = self._server_backend._client
        if client is None:
            logger.warning(
                "SwitchlySDKFlagProvider: HTTP client not ready — skipping initial flag fetch"
            )
            return
        ...
        try:
            ...
            logger.warning("SwitchlySDKFlagProvider: could not fetch flags from server")
        ...
        try:
            ...
            logger.warning("SwitchlySDKFlagProvider: could not fetch segments from server")

        logger.info(
            "SwitchlySDKFlagProvider: loaded %d flag(s), %d segment(s) from server",
            len(self._flags),
            len(self._segments),
        )
                ...
                if etype == "flag_updated":
                    ...
                    if raw is not None:
                        try:
                            ...
                            self._flags[flag.key] = flag
                            logger.debug(
                                "SwitchlySDKFlagProvider: flag hot-reloaded — %s", flag.key
                            )
                        except Exception:  # noqa: BLE001
                            pass
                    ...
                    logger.debug("SwitchlySDKFlagProvider: flag removed — %s", key)
                    ...
                    if raw is not None:
                        try:
                            ...
                            self._segments[seg.key] = seg
                            logger.debug(
                                "SwitchlySDKFlagProvider: segment hot-reloaded — %s", seg.key
                            )
                        except Exception:  # noqa: BLE001
                            pass
                    ...
                    logger.debug("SwitchlySDKFlagProvider: segment removed — %s", key)

        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("SwitchlySDKFlagProvider: SSE watch error")
