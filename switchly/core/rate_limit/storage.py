...
and expiry.  This module wires it to switchly's backend infrastructure.
...
auto-selects the appropriate storage based on the active ``SwitchlyBackend``.
...
from switchly.core.rate_limit.models import RateLimitAlgorithm, RateLimitResult

logger = logging.getLogger(__name__)

# How long before the window ends to report 0 remaining (avoids negative).
_RATE_LIMIT_NAMESPACE = "switchly:ratelimit"
...
class RateLimitStorage(ABC):
    ...
    its API into the simpler interface that ``SwitchlyRateLimiter`` needs.
    """
    ...
    async def increment(
        ...
        key:
            ...
            ``"switchly:ratelimit:GET:/api/payments:192.168.1.1"``).
        limit:
...
class MemoryRateLimitStorage(RateLimitStorage):
    ...
    def __init__(
        ...
        logger.debug(
            "switchly: MemoryRateLimitStorage initialised. "
...
class RedisRateLimitStorage(RateLimitStorage):
    ...
    async def reset(self, key: str) -> None:
        ...
        if redis_client is None:
            logger.warning("switchly: RedisRateLimitStorage.reset: cannot access redis client")
            return
        ...
        try:
            ...
            logger.warning("switchly: rate limit reset failed: %s", exc)
...
class FileRateLimitStorage(RateLimitStorage):
    """In-memory counters with periodic snapshot to the switchly state file.
    ...
    **Single-worker limitation**: each worker has its own counter because
    the in-memory counters are per-process.  ``SwitchlyProductionWarning``
    ...
    ----------
    file_path:
        Path to the switchly state file (same file used by ``FileBackend``).
    ...
    def __init__(
        ...
        warnings.warn(self._WARNING_MESSAGE, SwitchlyProductionWarning, stacklevel=2)
        from limits.storage import MemoryStorage
    ...
    async def startup(self) -> None:
        ...
        Called once by ``SwitchlyEngine`` after the storage is wired up,
        ...
        if self._snapshot_task is None or self._snapshot_task.done():
            try:
                ...
                self._snapshot_task = loop.create_task(
                    ...
                    name="switchly-rate-limit-snapshot",
                )
            except RuntimeError:
    ...
    async def flush_snapshot(self) -> None:
        """Write the current counter state to the switchly state file."""
        async with self._lock:
    ...
    async def _write_snapshot(self, snapshot: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning("switchly: rate limit snapshot write failed: %s", exc)
    ...
    async def _restore_from_snapshot(self) -> None:
        ...
        try:
            ...
            logger.warning("switchly: rate limit snapshot restore failed: %s", exc)
            return
        ...
        for key, entry in snapshot.items():
            try:
                ...
                logger.debug("switchly: skipping corrupt snapshot entry %r: %s", key, exc)
    ...
    async def increment(
        ...
        # When used through SwitchlyEngine, startup() already started the task
...
def create_rate_limit_storage(
    ...
    4. If main backend is ``FileBackend``, create ``FileRateLimitStorage``
       (same file path, emits ``SwitchlyProductionWarning``).

    Parameters
    ----------
    backend:
        The main ``SwitchlyBackend`` used for route state.
    rate_limit_backend:
    ...
    if isinstance(effective, FileBackend):
        file_path = str(getattr(effective, "_path", "switchly_state.json"))
    ...
    logger.warning(
        "switchly: unknown backend type %r — using MemoryRateLimitStorage for rate limits",
