"""Rate limiting data models for switchly.
...
class RateLimitAlgorithm(StrEnum):
    ...
    switchly never reimplements counting logic — this enum selects
    ...
    **Use this when burst smoothing matters more than predictability.**
    ...
    strategy.  switchly maps this to ``MOVING_WINDOW`` which provides
...
class RateLimitPolicy(BaseModel):
    ...
    Registered by ``SwitchlyRouter`` at startup and enforced by
    ``SwitchlyRateLimiter`` on every matching request.
    """
