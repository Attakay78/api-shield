"""SwitchlyRateLimiter — orchestrates rate limit checks against policies.

The limiter is the single entry point for rate limit enforcement.
``SwitchlyEngine.check()`` calls ``SwitchlyRateLimiter.check()`` when a policy
...
from switchly.core.rate_limit.keys import (
    KeyExtractor,
    handle_missing_key,
    is_exempt,
...
from switchly.core.rate_limit.storage import RateLimitStorage

logger = logging.getLogger(__name__)

# Prefix used for all rate limit storage keys to avoid collisions.
_KEY_PREFIX = "switchly:ratelimit"


class SwitchlyRateLimiter:
