"""SwitchlyServerBackend — remote backend that delegates to a Switchly Server.

Route states are cached locally so ``get_state()`` never touches the
network.  An SSE connection keeps the cache fresh whenever the Switchly
...
Or use the higher-level :class:`~switchly.sdk.SwitchlySDK` which wires
...
from switchly.core.backends.base import _GLOBAL_KEY, SwitchlyBackend
from switchly.core.models import AuditEntry, GlobalMaintenanceConfig, RouteState

logger = logging.getLogger(__name__)


class SwitchlyServerBackend(SwitchlyBackend):
    """Backend that enforces rules from a remote Switchly Server.
    ...
    ----------
    server_url:
        Base URL of the Switchly Server, including any mount prefix.
        Example: ``http://switchly-server:9000`` or
        ``http://myapp.com/switchly``.
    app_id:
        Unique identifier for this service.  Shown in the Switchly Server
    ...
    password:
        Switchly Server password.  Used with ``username`` for auto-login.
    reconnect_delay:
    ...
    def __init__(
        ...
        # path so duplicates (e.g. from SwitchlyRouter + SDK scan) are collapsed.
        ...
        # from SwitchlyRouter, from SDK route scan, from any ordering of startup
        ...
        # which invalidates the apply_switchly_to_openapi cache on every SSE update.
    ...
    def _headers(self) -> dict[str, str]:
        h: dict[str, str] = {"X-Switchly-App-Id": self._app_id}
        if self._token:
            h["X-Switchly-Token"] = self._token
        return h
    ...
    async def startup(self) -> None:
        """Connect to the Switchly Server, sync route states, start SSE listener."""
        ...
        self._sse_task = asyncio.create_task(
            self._sse_loop(),
            name=f"switchly-server-backend-sse[{self._app_id}]",
        )
    ...
    async def _auto_login(self) -> None:
        ...
        include the ``X-Switchly-Token`` header automatically.  Logs a
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.info("SwitchlyServerBackend[%s]: auto-login succeeded", self._app_id)
            else:
                logger.warning(
                    "SwitchlyServerBackend[%s]: auto-login failed (%s) — "
            ...
            logger.warning(
                "SwitchlyServerBackend[%s]: auto-login request failed — "
                "proceeding unauthenticated (fail-open)",
                self._app_id,
                exc_info=True,
            )
    ...
    async def _sync_from_server(self) -> None:
        """Pull all current route states and RL policies from the Switchly Server.
        ...
        try:
            ...
            logger.info(
                "SwitchlyServerBackend[%s]: synced %d route(s) from %s",
                self._app_id,
                len(self._cache),
                self._base_url,
            )
            ...
            logger.warning(
                "SwitchlyServerBackend[%s]: initial sync from %s failed — "
        ...
        try:
            ...
            logger.debug(
                "SwitchlyServerBackend[%s]: RL policy sync failed (non-fatal): %s",
                self._app_id,
                exc,
            )

    async def _flush_pending(self) -> None:
        """Push locally-registered new routes to the Switchly Server as one batch.

        Called by :class:`~switchly.sdk.SwitchlySDK` after route discovery at
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "SwitchlyServerBackend[%s]: registered %d new route(s) with server",
                    self._app_id,
                    len(batch),
                )
            except Exception as exc:  # noqa: BLE001
                logger.warning(
                    "SwitchlyServerBackend[%s]: failed to register %d route(s) with server: %s",
                    self._app_id,
                    len(batch),
                    exc,
                )
    ...
    async def _sse_loop(self) -> None:
        ...
        while True:
            try:
                ...
                logger.warning(
                    "SwitchlyServerBackend[%s]: SSE disconnected (%s), reconnecting in %.1fs",
                    self._app_id,
                    exc,
    ...
    def _local_path(state: RouteState) -> str:
        ...
        Routes registered by this SDK are stored on the Switchly Server with
        ...
        async with self._client.stream("GET", "/api/sdk/events") as resp:
            async for line in resp.aiter_lines():
                ...
                try:
                    ...
                    logger.debug(
                        "SwitchlyServerBackend[%s]: failed to parse SSE payload: %s",
                        self._app_id,
                        exc,
                    )
                    continue
                ...
                if event_type == "state":
                    ...
                    self._cache[local_key] = state
                    logger.debug(
                        "SwitchlyServerBackend[%s]: cache updated — %s → %s",
                        self._app_id,
                        local_key,
                        state.status,
                    )
                    ...
                    _svc_global_key = f"__switchly:svc_global:{self._app_id}__"
                    ...
                    if action == "set":
                        ...
                        logger.debug(
                            "SwitchlyServerBackend[%s]: RL policy set — %s", self._app_id, key
                        )
                    elif action == "delete":
                        ...
                        logger.debug(
                            "SwitchlyServerBackend[%s]: RL policy deleted — %s", self._app_id, key
                        )
                    ...
                    if key and flag_data is not None:
                        ...
                        logger.debug(
                            "SwitchlyServerBackend[%s]: flag cache updated — %s",
                            self._app_id,
                            key,
                        )
                    ...
                    if key:
                        ...
                        logger.debug(
                            "SwitchlyServerBackend[%s]: flag deleted — %s", self._app_id, key
                        )
                    ...
                    if key and seg_data is not None:
                        ...
                        logger.debug(
                            "SwitchlyServerBackend[%s]: segment cache updated — %s",
                            self._app_id,
                            key,
                        )
                    ...
                    if key:
                        ...
                        logger.debug(
                            "SwitchlyServerBackend[%s]: segment deleted — %s", self._app_id, key
                        )
    ...
    # ------------------------------------------------------------------
    # SwitchlyBackend ABC — core state operations
    # ------------------------------------------------------------------
    ...
    async def set_state(self, path: str, state: RouteState) -> None:
        """Update local cache immediately; push to Switchly Server asynchronously.

        Sets ``state.service`` to this SDK's ``app_id`` and stores with
        the service-prefixed path on the Switchly Server so routes from
        ...
        if self._client is None or not self._startup_done:
            ...
            # registrations (e.g. from both SwitchlyRouter and SDK scan) collapse
    ...
    async def _push_state(self, state: RouteState) -> None:
        ...
        try:
            ...
            logger.debug(
                "SwitchlyServerBackend[%s]: failed to push state for %s: %s",
                self._app_id,
                state.path,
                exc,
            )
    ...
    async def write_audit(self, entry: AuditEntry) -> None:
        """Forward audit entry to the Switchly Server (fire-and-forget)."""
        if self._client is not None:
    ...
    async def _push_audit(self, entry: AuditEntry) -> None:
        ...
        try:
            ...
            logger.debug(
                "SwitchlyServerBackend[%s]: failed to push audit entry: %s",
                self._app_id,
                exc,
            )
    ...
    async def get_audit_log(
        ...
        """Fetch audit log from the Switchly Server."""
        if self._client is None:
            return []
        try:
            ...
            logger.warning(
                "SwitchlyServerBackend[%s]: failed to fetch audit log: %s",
                self._app_id,
                exc,
            )
            return []
    ...
    async def subscribe(self) -> AsyncIterator[RouteState]:
        """Yield every route-state change pushed by the Switchly Server via SSE.
        ...
        ``apply_switchly_to_openapi``'s cache is invalidated and ``/docs``
    ...
    async def get_global_config(self) -> GlobalMaintenanceConfig:
        ...
        2. Service-specific maintenance (``__switchly:svc_global:<app_id>__``)
        ...
        svc_key = f"__switchly:svc_global:{self._app_id}__"
        try:
    ...
    async def get_registered_paths(self) -> set[str]:
        ...
        stripped) so that routes registered by ``SwitchlyRouter`` as
        ...
        keys = {k for k in self._cache.keys() if not k.startswith("__switchly:")}
        bare: set[str] = set()
    ...
    async def set_rate_limit_policy(
        ...
        """Update local RL policy cache (actual storage lives on the Switchly Server)."""
        key = f"{method.upper()}:{path}"
    ...
    async def load_all_flags(self) -> list[Any]:
        """Return all feature flags cached from the Switchly Server."""
        return list(self._flag_cache.values())

    async def load_all_segments(self) -> list[Any]:
        """Return all segments cached from the Switchly Server."""
