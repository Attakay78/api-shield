"""Abstract base class defining the backend contract for switchly."""
...
_GLOBAL_KEY = "__switchly:global__"
...
class SwitchlyBackend(ABC):
    ...
    async def startup(self) -> None:
        """Called by ``SwitchlyEngine`` on startup.
    ...
    async def shutdown(self) -> None:
        """Called by ``SwitchlyEngine`` on shutdown.
    ...
    async def try_claim_webhook_dispatch(self, dedup_key: str, ttl_seconds: int = 60) -> bool:
        ...
        Called by ``SwitchlyEngine._fire_webhooks`` before dispatching to any
    ...
    async def subscribe_global_config(self) -> AsyncIterator[None]:
        ...
        method.  Others raise ``NotImplementedError`` — ``SwitchlyEngine.start()``
    ...
    async def get_registered_paths(self) -> set[str]:
        ...
        Used by ``SwitchlyEngine.register_batch()`` to detect already-registered
        routes without re-querying the full state list.  The default
        implementation derives the set from ``list_states()``; backends that
        store routes under transformed keys (e.g. ``SwitchlyServerBackend`` which
    ...
    async def get_global_rate_limit_policy(self) -> dict[str, Any] | None:
        ...
        _GLOBAL_RL_KEY = "__switchly:global_rl__"
        try:
    ...
    # Stored as a sentinel with path ``__switchly:svc_rl:{service}__``.
    ...
    async def get_service_rate_limit_policy(self, service: str) -> dict[str, Any] | None:
        ...
        key = f"__switchly:svc_rl:{service}__"
        try:
    ...
    async def get_all_service_rate_limit_policies(self) -> dict[str, dict[str, Any]]:
        ...
        for state in await self.list_states():
            if state.path.startswith("__switchly:svc_rl:") and state.path.endswith("__"):
                svc = state.path[len("__switchly:svc_rl:") : -2]
                try:
    ...
    async def subscribe_rate_limit_policy(self) -> AsyncIterator[dict[str, Any]]:
        ...
        Others raise ``NotImplementedError`` — ``SwitchlyEngine.start()`` treats
