"""Redis backend for switchly.

Uses ``redis.asyncio`` for all I/O.  Supports multi-instance deployments
via pub/sub on the ``switchly:changes`` channel.

Key schema
----------
``switchly:state:{path}``        — JSON-serialized ``RouteState``
``switchly:route-index``         — Redis Set of all registered route paths
                                  (replaces dangerous ``KEYS`` scans with safe
                                  O(N) ``SMEMBERS`` that does not block the server)
``switchly:audit``               — Redis list, newest-first (LPUSH + LTRIM to 1000)
``switchly:audit:path:{path}``   — Per-path audit list for O(limit) filtered queries
                                  instead of fetching all 1000 entries to filter in Python
``switchly:changes``             — pub/sub channel for live state updates

Performance notes
-----------------
*  ``list_states()`` uses ``SMEMBERS switchly:route-index`` + ``MGET`` instead
   of ``KEYS switchly:state:*``.  ``KEYS`` is an O(keyspace) blocking command
...
*  ``get_audit_log(path=X)`` reads directly from ``switchly:audit:path:X``
...
_AUDIT_KEY = "switchly:audit"
_RATE_LIMIT_HITS_KEY = "switchly:ratelimit:hits"
_ROUTE_INDEX_KEY = "switchly:route-index"
_CHANGES_CHANNEL = "switchly:changes"
...
_GLOBAL_INVALIDATE_CHANNEL = "switchly:global_invalidate"
_RL_POLICY_CHANNEL = "switchly:rl-policy-change"
_MAX_AUDIT_ENTRIES = 1000


def _state_key(path: str) -> str:
    return f"switchly:state:{path}"


def _audit_path_key(path: str) -> str:
    """Per-path audit list key for O(limit) filtered audit queries."""
    return f"switchly:audit:path:{path}"


class RedisBackend(SwitchlyBackend):
    ...
    async def get_state(self, path: str) -> RouteState:
        ...
        try:
            ...
            logger.error("switchly: redis get_state error for %r: %s", path, exc)
            raise
    ...
    async def set_state(self, path: str, state: RouteState) -> None:
        ...
        try:
            ...
            logger.error("switchly: redis set_state error for %r: %s", path, exc)
            raise
    ...
    async def delete_state(self, path: str) -> None:
        ...
        try:
            ...
            logger.error("switchly: redis delete_state error: %s", exc)
            raise

    async def list_states(self) -> list[RouteState]:
        """Return all registered route states.

        Uses ``SMEMBERS switchly:route-index`` + ``MGET`` instead of the
        dangerous ``KEYS switchly:state:*`` pattern.  ``KEYS`` is an O(keyspace)
        ...
        try:
            ...
            logger.error("switchly: redis list_states error: %s", exc)
            raise
    ...
    async def write_audit(self, entry: AuditEntry) -> None:
        ...
        try:
            ...
            logger.error("switchly: redis write_audit error: %s", exc)
            raise
    ...
    async def get_audit_log(self, path: str | None = None, limit: int = 100) -> list[AuditEntry]:
        ...
        *limit* entries via a single ``LRANGE`` call, eliminating the
        ...
        try:
            ...
            logger.error("switchly: redis get_audit_log error: %s", exc)
            raise
            ...
            async with r.pubsub() as pubsub:
                ...
                async for message in pubsub.listen():
                    ...
                    try:
                        ...
                        logger.warning("switchly: redis subscribe parse error: %s", exc)
    ...
    async def set_global_config(self, config: GlobalMaintenanceConfig) -> None:
        ...
        ``set_state`` and publishes to ``switchly:changes``), then
        additionally publishes a lightweight ``"1"`` signal to
        ``switchly:global_invalidate``.  Any other instance running
        ...
        try:
            ...
            logger.warning("switchly: failed to publish global config invalidation signal: %s", exc)
    ...
    async def try_claim_webhook_dispatch(self, dedup_key: str, ttl_seconds: int = 60) -> bool:
        ...
        The key ``switchly:webhook:dedup:{dedup_key}`` is written with
        ...
        try:
            ...
            logger.warning("switchly: redis webhook dedup check failed (%s) — failing open", exc)
    ...
    async def write_rate_limit_hit(self, hit: RateLimitHit) -> None:
        ...
        try:
            ...
            logger.warning("switchly: redis write_rate_limit_hit error: %s", exc)
    ...
    async def get_rate_limit_hits(
        ...
        from switchly.core.rate_limit.models import RateLimitHit as RLHit

        try:
            async with self._client() as r:
                raws: list[str] = await r.lrange(_RATE_LIMIT_HITS_KEY, 0, -1)  # type: ignore[misc]
        except Exception as exc:
            logger.warning("switchly: redis get_rate_limit_hits error: %s", exc)
            return []
    ...
    async def set_rate_limit_policy(
        ...
        all other instances via ``switchly:rl-policy-change``."""
        key = f"{method.upper()}:{path}"
        redis_key = f"switchly:rlpolicy:{key}"
        index_key = "switchly:rl-policy-index"
        ...
        try:
            ...
            logger.warning("switchly: redis set_rate_limit_policy error: %s", exc)
    ...
    async def get_rate_limit_policies(self) -> list[dict[str, Any]]:
        ...
        try:
            async with self._client() as r:
                ...
                if not keys:
                    return []
                redis_keys = [f"switchly:rlpolicy:{k}" for k in keys]
            ...
            logger.warning("switchly: redis get_rate_limit_policies error: %s", exc)
            return []
    ...
    async def delete_rate_limit_policy(self, path: str, method: str) -> None:
        ...
        try:
            ...
            logger.warning("switchly: redis delete_rate_limit_policy error: %s", exc)

    async def subscribe_global_config(self) -> AsyncIterator[None]:
        """Yield ``None`` whenever the global maintenance config changes.

        Subscribes to ``switchly:global_invalidate``.  Each message
        ...
        The generator runs indefinitely — callers (``SwitchlyEngine``)
            ...
            async with r.pubsub() as pubsub:
                ...
                async for message in pubsub.listen():
                    ...
                    try:
                        ...
                        logger.warning("switchly: redis rl-policy-change parse error: %s", exc)
