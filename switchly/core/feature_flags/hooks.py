"""Built-in OpenFeature hooks for switchly.
...
``AuditHook``   — records non-trivial evaluations in SwitchlyEngine's audit log.
...
class LoggingHook(Hook):
    ...
    def after(
        ...
        logger.debug(
            "switchly flag eval: key=%s variant=%s reason=%s",
    ...
    def error(
        ...
        logger.error(
            "switchly flag error: key=%s error=%s",
            hook_context.flag_key,
            exception,
        )
...
class AuditHook(Hook):
    """Record flag evaluations in SwitchlyEngine's audit log.
    ...
    ----------
    engine:
        The ``SwitchlyEngine`` instance to write audit entries to.
    """
