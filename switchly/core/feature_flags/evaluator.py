...
class FlagEvaluator:
    ...
    def evaluate(
        ...
        *,
        ...
        if _depth > _MAX_PREREQ_DEPTH:
            logger.error(
                "switchly flags: prerequisite depth limit reached for flag '%s'. "
        ...
        for prereq in flag.prerequisites:
            ...
            if prereq_flag is None:
                logger.warning(
                    "switchly flags: prerequisite flag '%s' not found "
    ...
    def _apply_operator(self, op: Operator, actual: Any, values: list[Any]) -> bool:
        ...
        match op:
            ...
            case _:
                logger.warning("switchly flags: unknown operator '%s'", op)
                return False
    ...
    def _in_segment(
        ...
        *,
        ...
        if seg is None:
            logger.warning(
                "switchly flags: segment '%s' not found — treating as empty.",
                segment_key,
            )
            return False
    ...
    def _resolve_rule_variation(
        ...
        logger.warning(
            "switchly flags: rule '%s' on flag '%s' has neither variation "
...
def _safe_regex(pattern: str, string: str) -> bool:
    ...
    try:
        ...
        logger.warning("switchly flags: invalid regex '%s': %s", pattern, exc)
        return False
...
def _semver_op(actual: Any, threshold: Any, op: str) -> bool:
    ...
    try:
        ...
        logger.warning(
            "switchly flags: semver operators require 'packaging'. "
            "Install with: pip install switchly[flags]"
        )
    except Exception:  # noqa: BLE001
        logger.warning(
            "switchly flags: semver comparison failed for values '%s' and '%s'.",
            actual,
            threshold,
        )
    return False
