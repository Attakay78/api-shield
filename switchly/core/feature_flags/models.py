"""Feature flag data models for switchly.
...
class FeatureFlag(BaseModel):
    ...
    Stored in ``SwitchlyBackend`` alongside ``RouteState``.
    Backend storage key convention: ``switchly:flag:{key}``.
...
class EvaluationContext(BaseModel):
    ...
    ``SwitchlyFeatureClient.get_*_value()``.
