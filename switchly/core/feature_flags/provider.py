"""SwitchlyOpenFeatureProvider — native OpenFeature provider backed by SwitchlyBackend.
...
from switchly.core.feature_flags._context import from_of_context
...
# Map Switchly EvaluationReason → OpenFeature Reason string
...
class SwitchlyOpenFeatureProvider(AbstractProvider):
    """OpenFeature-compliant provider backed by ``SwitchlyBackend``.
    ...
    ----------
    backend:
        The ``SwitchlyBackend`` instance (Memory, File, or Redis).
        Must be the same instance passed to ``SwitchlyEngine``.
    """
    ...
    async def _load_all(self) -> None:
        ...
        try:
            ...
            logger.debug(
                "switchly flags: backend does not support flag storage yet. "
    ...
    def _resolve(
        ...
        try:
            ...
            logger.exception("switchly flags: evaluation error for '%s'", flag_key)
            return FlagResolutionDetails(
                value=default_value,
                reason=Reason.ERROR,
