...
``switchly/core/feature_flags/models.py`` and ``evaluator.py`` are pure
...
``switchly.core.feature_flags`` namespace (``__init__.py``) and the
...
def _require_flags() -> None:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or:            uv pip install 'switchly[flags]'"
        ) from None
