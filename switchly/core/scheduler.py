...
class MaintenanceScheduler:
    ...
    ----------
    engine:
        The ``SwitchlyEngine`` instance used to activate and deactivate
    ...
    async def schedule(
        ...
        task = asyncio.create_task(
            ...
            name=f"switchly-scheduler:{path}",
        )
        self._tasks[path] = task
        task.add_done_callback(lambda t: self._on_task_done(path, t))
        logger.info(
            "switchly: scheduled maintenance for %r from %s to %s",
            path,
    ...
    async def restore_from_backend(self, actor: str = "scheduler") -> None:
        ...
        try:
            ...
            logger.exception("switchly: failed to restore scheduled windows from backend")
            return
        ...
        for state in states:
            ...
            if existing is not None and not existing.done():
                continue
            logger.info("switchly: restoring scheduled window for %r from backend", state.path)
        ...
        async def _poll() -> None:
            while True:
                ...
                try:
                    ...
                    logger.exception("switchly: scheduler poll cycle failed")

        self._poll_task = asyncio.create_task(_poll(), name="switchly-scheduler-poll")
        logger.info("switchly: scheduler polling started (interval=%.0fs)", interval_seconds)
    ...
    def stop_polling(self) -> None:
        ...
        if self._poll_task is not None and not self._poll_task.done():
            ...
            logger.info("switchly: scheduler polling stopped")
    ...
    async def _run_window(self, path: str, window: MaintenanceWindow) -> None:
        ...
        try:
            ...
            logger.info("switchly: maintenance window opened for %r", path)
        except Exception:
            logger.exception("switchly: failed to activate maintenance for %r", path)
            return
        ...
        try:
            ...
            logger.info("switchly: maintenance window closed for %r", path)
        except Exception:
            logger.exception("switchly: failed to deactivate maintenance for %r", path)
    ...
    def _on_task_done(self, path: str, task: asyncio.Task[None]) -> None:
        ...
        if not task.cancelled() and task.exception() is not None:
            logger.error(
                "switchly: scheduler task for %r raised an exception: %s",
                path,
                task.exception(),
            )
