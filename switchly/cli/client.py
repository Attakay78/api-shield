"""Async HTTP client for the Switchly admin REST API.

All CLI commands use :func:`make_client` to obtain a :class:`SwitchlyClient`
configured from ``~/.switchly/config.json``.  An optional ``transport``
parameter makes the client fully testable without a live server::

    transport = httpx.ASGITransport(app=admin_app)
    client = SwitchlyClient(base_url="http://test", transport=transport)
"""
...
class SwitchlyClientError(Exception):
    """Raised when the Switchly server returns an error response."""
...
class SwitchlyClient:
    """Async HTTP client for the Switchly admin REST API.

    Parameters
    ----------
    base_url:
        Base URL of the mounted :func:`~switchly.admin.app.SwitchlyAdmin` app
        (e.g. ``http://localhost:8000/switchly``).
    token:
    ...
    def _headers(self) -> dict[str, str]:
        headers: dict[str, str] = {"X-Switchly-Platform": "cli"}
        if self._token:
            headers["X-Switchly-Token"] = self._token
        return headers
    ...
    def _check(self, resp: httpx.Response) -> Any:
        ...
        if resp.status_code >= 400:
            try:
                ...
                matches = None
            raise SwitchlyClientError(err, resp.status_code, ambiguous_matches=matches)
        return resp.json()
...
def make_client(
    ...
    """Create a :class:`SwitchlyClient` from the local CLI config.

    Reads ``server_url`` and ``auth.token`` from ``~/.switchly/config.json``.
    ...
    return SwitchlyClient(base_url=server_url, token=token, transport=transport)
