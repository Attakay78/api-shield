"""switchly CLI — ``switchly`` command.

The CLI is a **thin HTTP client** that communicates with a running
:func:`~switchly.admin.app.SwitchlyAdmin` instance mounted on your FastAPI
application.  It never touches the backend directly.

Quick start
-----------
1. Mount :class:`~switchly.admin.app.SwitchlyAdmin` on your app::

       admin = SwitchlyAdmin(engine=engine, auth=("admin", "secret"))
...
3. Authenticate (when auth is configured)::
       ...
       # or: switchly login --username admin --password secret

4. Manage routes::

       switchly status
       switchly disable /api/payments --reason "migration"
       switchly enable /api/payments

Authentication
--------------
Credentials are stored in ``~/.switchly/config.json`` with an expiry
timestamp.  After expiry you must re-authenticate with ``switchly login``.

Token expiry is controlled server-side via ``SwitchlyAdmin(token_expiry=…)``.
"""
...
from switchly.cli import config as _cfg
from switchly.cli.client import SwitchlyClient, SwitchlyClientError, make_client
...
cli = typer.Typer(
    name="switchly",
    help="switchly — route lifecycle management CLI",
...
def _run(coro_fn: object) -> None:
    ...
    except SwitchlyClientError as exc:
        if exc.status_code == 401:
            err_console.print(
                "[red]Error:[/red] Authentication required.\n"
                "  Run: [bold]switchly login <username>[/bold]"
            )
...
def login(
    ...
    """Authenticate with the Switchly admin server and store a token locally."""
    ...
    async def _run_login() -> None:
        ...
        client = SwitchlyClient(base_url=_cfg.require_server_url())
...
def config_set_url(
    url: str = typer.Argument(..., help="Base URL of the SwitchlyAdmin mount point."),
) -> None:
    """Set the Switchly admin server URL."""
...
def status(
    ...
    service: str | None = typer.Option(
        ...
        "--service",
        "-s",
        envvar="SWITCHLY_SERVICE",
        help="Filter to routes for a specific service. Falls back to SWITCHLY_SERVICE env var.",
    ),
    ...
    """Show current switchly status for all routes (or one route)."""
...
def list_services_cmd() -> None:
    """List all services that have registered routes with this Switchly Server."""
...
def current_service_cmd() -> None:
    """Show the active service context (set via SWITCHLY_SERVICE env var)."""
    import os

    svc = os.environ.get("SWITCHLY_SERVICE", "")
    if svc:
        console.print(
            f"Active service: [cyan bold]{svc}[/cyan bold]  [dim](from SWITCHLY_SERVICE)[/dim]"
        )
    else:
        console.print(
            "[dim]No active service set.[/dim]\n"
            "Set one with: [cyan]export SWITCHLY_SERVICE=<service-name>[/cyan]"
        )
...
def enable(
    ...
    service: str | None = typer.Option(
        ...
        "-s",
        envvar="SWITCHLY_SERVICE",
        help="Service name (SDK multi-service mode). Falls back to SWITCHLY_SERVICE env var.",
    ),
) -> None:
...
# ---------------------------------------------------------------------------
# Global maintenance command group  (switchly global ...)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Env gate command group  (switchly env ...)
# ---------------------------------------------------------------------------
...
def env_set(
    ...
    \b
      switchly env set /api/debug dev
      switchly env set /api/internal dev staging
    """
...
# ---------------------------------------------------------------------------
# Rate limits command group  (switchly rate-limits ...)
# ---------------------------------------------------------------------------
...
def rl_reset(
    ...
    \b
      switchly rl reset GET:/api/items
      switchly rl reset /api/items        (resets all methods)
    """
    import base64
...
def rl_set(
    ...
    \b
      switchly rl set GET:/api/items 100/minute
      switchly rl set POST:/api/pay 10/second --algorithm fixed_window --key ip
    """
    key = _parse_route(route)
    if ":" in key and not key.startswith("/"):
...
def rl_delete(
    ...
    \b
      switchly rl delete GET:/api/items
      switchly rl delete /api/items       (defaults to GET)
    """
    key = _parse_route(route)
    if ":" in key and not key.startswith("/"):
...
# ---------------------------------------------------------------------------
# Global rate limit command group  (switchly global-rate-limit ...)
# ---------------------------------------------------------------------------
...
def grl_set(
    ...
    \b
      ...
      switchly grl set 500/minute --key ip --exempt /health --exempt GET:/api/internal
      switchly grl set 200/hour --algorithm sliding_window --burst 20
    """
...
# ---------------------------------------------------------------------------
# Per-service rate limit command group  (switchly service-rate-limit ...)
# ---------------------------------------------------------------------------
...
def srl_set(
    ...
    \b
      ...
      switchly srl set orders-service 500/minute --key ip --exempt /health
      switchly srl set auth-service 200/hour --algorithm sliding_window --burst 20
    """
...
# ---------------------------------------------------------------------------
# Per-service maintenance command group  (switchly sm ...)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Feature flags command group  (switchly flags ...)
# ---------------------------------------------------------------------------
...
def flags_create(
    ...
    \b
      switchly flags create new_checkout --name "New Checkout Flow"
      switchly flags create dark_mode --name "Dark Mode" --type boolean
    """
...
def flags_eval(
    ...
    \b
      switchly flags eval new_checkout --key user_123 --attr role=admin --attr plan=pro
    """
...
def flags_edit(
    ...
    \b
      switchly flags edit dark_mode --name "Dark Mode v2"
      switchly flags edit dark_mode --off-variation off --fallthrough control
    """
...
def flags_add_rule(
    ...
    Segment-based rule (most common):
      switchly flags add-rule my-flag --variation on --segment beta-users

    Custom attribute rule:
      switchly flags add-rule my-flag --variation on \
...
def flags_remove_rule(
    ...
    \b
      switchly flags remove-rule my-flag --rule-id <uuid>

    Use 'switchly flags targeting my-flag' to list rule IDs.
    """
...
# ---------------------------------------------------------------------------
# Prerequisites commands  (switchly flags add-prereq / remove-prereq)
# ---------------------------------------------------------------------------
...
def flags_add_prereq(
    ...
    \b
      switchly flags add-prereq my-flag --flag auth-flag --variation on
...
def flags_remove_prereq(
    ...
    \b
      switchly flags remove-prereq my-flag --flag auth-flag
    """
...
# ---------------------------------------------------------------------------
# Individual targets commands  (switchly flags target / untarget)
# ---------------------------------------------------------------------------
...
def flags_target(
    ...
    \b
      switchly flags target my-flag --variation on --keys user_123,user_456
...
def flags_untarget(
    ...
    \b
      switchly flags untarget my-flag --variation on --keys user_123
    """
...
# ---------------------------------------------------------------------------
# Segments command group  (switchly segments ...)
# ---------------------------------------------------------------------------
...
def segments_exclude(
    ...
    \b
      switchly segments exclude beta_users --context-key user_789
    """
...
def segments_add_rule(
    ...
    Examples:
      ...
      switchly segments add-rule beta_users --attribute country --operator is --values GB
      switchly segments add-rule beta_users --attribute email --operator ends_with \\
...
def segments_remove_rule(
    ...
    Use 'switchly segments get beta_users' to list rule IDs.
    """
