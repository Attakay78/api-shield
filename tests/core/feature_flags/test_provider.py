"""Tests for shield.core.feature_flags.provider — ShieldOpenFeatureProvider."""
...
class TestProviderMetadata:
    def test_name(self):
        provider = ShieldOpenFeatureProvider(_FakeBackend())
        assert provider.get_metadata().name == "shield"
...
class TestInitialize:
    async def test_loads_flags_and_segments(self):
        ...
        provider = ShieldOpenFeatureProvider(backend)
    ...
    async def test_graceful_on_missing_backend_support(self):
        provider = ShieldOpenFeatureProvider(_NoFlagBackend())
...
class TestResolveBooleanDetails:
    ...
    async def test_disabled_flag_returns_off_variation(self):
        ...
        provider = ShieldOpenFeatureProvider(_FakeBackend(flags=[flag]))
        await provider._load_all()
