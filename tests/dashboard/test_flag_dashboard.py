...
@pytest.fixture
def admin_app(engine: ShieldEngine) -> object:
    """ShieldAdmin with flags enabled, no auth."""
    return ShieldAdmin(engine=engine, auth=None, enable_flags=True)
...
@pytest.fixture
def admin_app_no_flags(engine: ShieldEngine) -> object:
    """ShieldAdmin with flags disabled."""
    return ShieldAdmin(engine=engine, auth=None, enable_flags=False)
...
class TestFlagsPage:
    ...
    async def test_flags_page_shows_flag_key(
        self, client: AsyncClient, engine: ShieldEngine
    ) -> None:
...
class TestFlagEvalForm:
    ...
    async def test_eval_form_triggers_event(self, client: AsyncClient) -> None:
        ...
        assert "shieldEvalDone" in trigger
        payload = trigger["shieldEvalDone"]
...
class TestFlagTargetsSave:
    async def test_targets_save_adds_keys(self, client: AsyncClient, engine: ShieldEngine) -> None:
