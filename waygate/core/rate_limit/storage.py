...
and expiry.  This module wires it to waygate's backend infrastructure.
...
auto-selects the appropriate storage based on the active ``WaygateBackend``.
...
from waygate.core.rate_limit.models import RateLimitAlgorithm, RateLimitResult

logger = logging.getLogger(__name__)

# How long before the window ends to report 0 remaining (avoids negative).
_RATE_LIMIT_NAMESPACE = "waygate:ratelimit"
...
class RateLimitStorage(ABC):
    ...
    its API into the simpler interface that ``WaygateRateLimiter`` needs.
    """
    ...
    async def increment(
        ...
        key:
            ...
            ``"waygate:ratelimit:GET:/api/payments:192.168.1.1"``).
        limit:
...
class MemoryRateLimitStorage(RateLimitStorage):
    ...
    def __init__(
        ...
        logger.debug(
            "waygate: MemoryRateLimitStorage initialised. "
...
class RedisRateLimitStorage(RateLimitStorage):
    ...
    async def reset(self, key: str) -> None:
        ...
        if redis_client is None:
            logger.warning("waygate: RedisRateLimitStorage.reset: cannot access redis client")
            return
        ...
        try:
            ...
            logger.warning("waygate: rate limit reset failed: %s", exc)
...
class FileRateLimitStorage(RateLimitStorage):
    """In-memory counters with periodic snapshot to the waygate state file.
    ...
    **Single-worker limitation**: each worker has its own counter because
    the in-memory counters are per-process.  ``WaygateProductionWarning``
    ...
    ----------
    file_path:
        Path to the waygate state file (same file used by ``FileBackend``).
    ...
    def __init__(
        ...
        warnings.warn(self._WARNING_MESSAGE, WaygateProductionWarning, stacklevel=2)
        from limits.storage import MemoryStorage
    ...
    async def startup(self) -> None:
        ...
        Called once by ``WaygateEngine`` after the storage is wired up,
        ...
        if self._snapshot_task is None or self._snapshot_task.done():
            try:
                ...
                self._snapshot_task = loop.create_task(
                    ...
                    name="waygate-rate-limit-snapshot",
                )
            except RuntimeError:
    ...
    async def flush_snapshot(self) -> None:
        """Write the current counter state to the waygate state file."""
        async with self._lock:
    ...
    async def _write_snapshot(self, snapshot: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning("waygate: rate limit snapshot write failed: %s", exc)
    ...
    async def _restore_from_snapshot(self) -> None:
        ...
        try:
            ...
            logger.warning("waygate: rate limit snapshot restore failed: %s", exc)
            return
        ...
        for key, entry in snapshot.items():
            try:
                ...
                logger.debug("waygate: skipping corrupt snapshot entry %r: %s", key, exc)
    ...
    async def increment(
        ...
        # When used through WaygateEngine, startup() already started the task
...
def create_rate_limit_storage(
    ...
    4. If main backend is ``FileBackend``, create ``FileRateLimitStorage``
       (same file path, emits ``WaygateProductionWarning``).

    Parameters
    ----------
    backend:
        The main ``WaygateBackend`` used for route state.
    rate_limit_backend:
    ...
    if isinstance(effective, FileBackend):
        file_path = str(getattr(effective, "_path", "waygate_state.json"))
    ...
    logger.warning(
        "waygate: unknown backend type %r — using MemoryRateLimitStorage for rate limits",
