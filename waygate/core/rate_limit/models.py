"""Rate limiting data models for waygate.
...
class RateLimitAlgorithm(StrEnum):
    ...
    waygate never reimplements counting logic — this enum selects
...
class RateLimitPolicy(BaseModel):
    ...
    Registered by ``WaygateRouter`` at startup and enforced by
    ``WaygateRateLimiter`` on every matching request.
    """
