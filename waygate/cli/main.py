"""waygate CLI — ``waygate`` command.

The CLI is a **thin HTTP client** that communicates with a running
:func:`~waygate.admin.app.WaygateAdmin` instance mounted on your FastAPI
application.  It never touches the backend directly.

Quick start
-----------
1. Mount :class:`~waygate.admin.app.WaygateAdmin` on your app::

       admin = WaygateAdmin(engine=engine, auth=("admin", "secret"))
...
3. Authenticate (when auth is configured)::
       ...
       waygate login admin          # prompts for password
       # or: waygate login --username admin --password secret

4. Manage routes::

       waygate status
       waygate disable /api/payments --reason "migration"
       waygate enable /api/payments

Authentication
--------------
Credentials are stored in ``~/.waygate/config.json`` with an expiry
timestamp.  After expiry you must re-authenticate with ``waygate login``.

Token expiry is controlled server-side via ``WaygateAdmin(token_expiry=…)``.
"""
...
from waygate.cli.client import WaygateClient, WaygateClientError, make_client
...
cli = typer.Typer(
    name="waygate",
    help="waygate — route lifecycle management CLI",
...
def _run(coro_fn: object) -> None:
    ...
    except WaygateClientError as exc:
        if exc.status_code == 401:
            err_console.print(
                "[red]Error:[/red] Authentication required.\n"
                "  Run: [bold]waygate login <username>[/bold]"
            )
...
def login(
    ...
    """Authenticate with the Waygate admin server and store a token locally."""
    ...
    async def _run_login() -> None:
        ...
        client = WaygateClient(base_url=_cfg.require_server_url())
...
def config_set_url(
    url: str = typer.Argument(..., help="Base URL of the WaygateAdmin mount point."),
) -> None:
    """Set the Waygate admin server URL."""
...
def status(
    ...
    service: str | None = typer.Option(
        ...
        "--service",
        "-s",
        envvar="WAYGATE_SERVICE",
        help="Filter to routes for a specific service. Falls back to WAYGATE_SERVICE env var.",
    ),
    ...
    """Show current waygate status for all routes (or one route)."""
...
def list_services_cmd() -> None:
    """List all services that have registered routes with this Waygate Server."""
...
def current_service_cmd() -> None:
    """Show the active service context (set via WAYGATE_SERVICE env var)."""
    import os

    svc = os.environ.get("WAYGATE_SERVICE", "")
    if svc:
        console.print(
            f"Active service: [cyan bold]{svc}[/cyan bold]  [dim](from WAYGATE_SERVICE)[/dim]"
        )
    else:
        console.print(
            "[dim]No active service set.[/dim]\n"
            "Set one with: [cyan]export WAYGATE_SERVICE=<service-name>[/cyan]"
        )
...
def enable(
    ...
    service: str | None = typer.Option(
        ...
        "-s",
        envvar="WAYGATE_SERVICE",
        help="Service name (SDK multi-service mode). Falls back to WAYGATE_SERVICE env var.",
    ),
) -> None:
...
# ---------------------------------------------------------------------------
# Global maintenance command group  (waygate global ...)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Env gate command group  (waygate env ...)
# ---------------------------------------------------------------------------
...
def env_set(
    ...
    \b
      waygate env set /api/debug dev
      waygate env set /api/internal dev staging
    """
...
# ---------------------------------------------------------------------------
# Rate limits command group  (waygate rate-limits ...)
# ---------------------------------------------------------------------------
...
def rl_reset(
    ...
    \b
      waygate rl reset GET:/api/items
      waygate rl reset /api/items        (resets all methods)
    """
    import base64
...
def rl_set(
    ...
    \b
      waygate rl set GET:/api/items 100/minute
      waygate rl set POST:/api/pay 10/second --algorithm fixed_window --key ip
    """
    key = _parse_route(route)
    if ":" in key and not key.startswith("/"):
...
def rl_delete(
    ...
    \b
      waygate rl delete GET:/api/items
      waygate rl delete /api/items       (defaults to GET)
    """
    key = _parse_route(route)
    if ":" in key and not key.startswith("/"):
...
# ---------------------------------------------------------------------------
# Global rate limit command group  (waygate global-rate-limit ...)
# ---------------------------------------------------------------------------
...
def grl_set(
    ...
    \b
      ...
      waygate grl set 500/minute --key ip --exempt /health --exempt GET:/api/internal
      waygate grl set 200/hour --algorithm sliding_window --burst 20
    """
...
# ---------------------------------------------------------------------------
# Per-service rate limit command group  (waygate service-rate-limit ...)
# ---------------------------------------------------------------------------
...
def srl_set(
    ...
    \b
      ...
      waygate srl set orders-service 500/minute --key ip --exempt /health
      waygate srl set auth-service 200/hour --algorithm sliding_window --burst 20
    """
...
# ---------------------------------------------------------------------------
# Per-service maintenance command group  (waygate sm ...)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Feature flags command group  (waygate flags ...)
# ---------------------------------------------------------------------------
...
def flags_create(
    ...
    \b
      waygate flags create new_checkout --name "New Checkout Flow"
      waygate flags create dark_mode --name "Dark Mode" --type boolean
    """
...
def flags_eval(
    ...
    \b
      waygate flags eval new_checkout --key user_123 --attr role=admin --attr plan=pro
    """
...
def flags_edit(
    ...
    \b
      waygate flags edit dark_mode --name "Dark Mode v2"
      waygate flags edit dark_mode --off-variation off --fallthrough control
    """
...
def flags_add_rule(
    ...
    Segment-based rule (most common):
      waygate flags add-rule my-flag --variation on --segment beta-users

    Custom attribute rule:
      waygate flags add-rule my-flag --variation on \
...
def flags_remove_rule(
    ...
    \b
      waygate flags remove-rule my-flag --rule-id <uuid>

    Use 'waygate flags targeting my-flag' to list rule IDs.
    """
...
# ---------------------------------------------------------------------------
# Prerequisites commands  (waygate flags add-prereq / remove-prereq)
# ---------------------------------------------------------------------------
...
def flags_add_prereq(
    ...
    \b
      waygate flags add-prereq my-flag --flag auth-flag --variation on
...
def flags_remove_prereq(
    ...
    \b
      waygate flags remove-prereq my-flag --flag auth-flag
    """
...
# ---------------------------------------------------------------------------
# Individual targets commands  (waygate flags target / untarget)
# ---------------------------------------------------------------------------
...
def flags_target(
    ...
    \b
      waygate flags target my-flag --variation on --keys user_123,user_456
...
def flags_untarget(
    ...
    \b
      waygate flags untarget my-flag --variation on --keys user_123
    """
...
# ---------------------------------------------------------------------------
# Segments command group  (waygate segments ...)
# ---------------------------------------------------------------------------
...
def segments_exclude(
    ...
    \b
      waygate segments exclude beta_users --context-key user_789
    """
...
def segments_add_rule(
    ...
    Examples:
      ...
      waygate segments add-rule beta_users --attribute country --operator is --values GB
      waygate segments add-rule beta_users --attribute email --operator ends_with \\
...
def segments_remove_rule(
    ...
    Use 'waygate segments get beta_users' to list rule IDs.
    """
